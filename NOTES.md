...
Update 2026-06-11: the Robertson teaching resume (JoanneHoarTeachingResume.pdf, used for the Robertson application) says "French - proficient." Three different claims now exist (advanced / proficient / intermediate). All current application materials use the conservative "intermediate" — ask Joanne which she wants as standard.
...
**Cross-check vs Robertson teaching resume (same day):** compared the package against `JoanneHoarTeachingResume.pdf` (the resume Joanne used to get the Robertson job). All facts consistent except French level (see Verification #5). One addition: the BJ Pipeline **neural network anomaly detection** work (verified in experience.xml) was missing from the AI SME resume — added to md + tex + PDF as an applied-AI industry credential.
